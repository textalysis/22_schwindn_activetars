...
def write(name, contents):
    with open('results_coreset_withoutseedset_3.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(contents))
...
CoreSet = Core.CoreSet(corpus = TREC_CoreSet, TARS = TARS_CoreSet, device = 'cuda:2')
...
for i in range(10):
    ...
    CoreSet.trainTARS(path = 'resources/taggers/CoreSet3')
    CoreSetAccuracy_seed.append(CoreSet.evaluateModel())
    #CoreSet.SelectData(TrainSetSize)
    #CoreSet.trainTARS(path = 'resources/taggers/CoreSet')
    #CoreSetAccuracy_seed.append(CoreSet.evaluateModel())
